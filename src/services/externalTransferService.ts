...
export const sendToExternalBank = async (
  ...
  try {
    ...
    // Validate reference against recent webhook deposit
    const deposit = await db('transactions')
      .where({ user_id: userId, type: 'deposit', reference })
      .orderBy('created_at', 'desc')
      .first();
    if (!deposit)
      throw new Error('No matching deposit found for this reference');
    ...
    // Log the transfer transaction locally
    const [transaction] = await db('transactions')
      .insert({
        user_id: userId,
        account_id: account.id,
